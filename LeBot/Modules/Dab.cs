...
{
    public class Dab : ModuleBase<SocketCommandContext>
    {
        [Command("dab")]
        public async Task dabAsync()
        {
            ...
            using (WebClient client = new WebClient())
            {
                ...
                client.DownloadFile(new Uri("http://192.168.1.141/bot/dab/?user=" + Context.Message.Author.Id + "&avatar=" + Context.Message.Author.AvatarId), @"C:\LeBotFiles\Images\" + Context.Message.Author.Id + "_dab.png");
                Console.WriteLine(" Pronto!");
            }
            
            var imageToSend = @"C:\LeBotFiles\Images\" + Context.Message.Author.Id + "_dab.png";
            int inUse = 1;
            int tryUse = 0;
            while (inUse != 0)
            {
